...
export function AddressPage({ onWeatherClick, onLiveClick, onBack }: AddressPageProps) {
  const [selectedAction, setSelectedAction] = useState<'route' | '2gis' | 'taxi' | 'bus'>('route');
      ...
      <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
        ...
        <div className="pb-6">
          <div className="space-y-4">
            {/* Address Header with Navigation */}
            <div className="bg-gray-50 rounded-2xl p-4">
              ...
              <div className="lg:hidden grid grid-cols-2 gap-2 mb-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 h-10 bg-gradient-to-r from-orange-500 to-orange-600 rounded-full shadow-sm hover:shadow-md transition-all active:scale-95"
                  type="button"
                >
                  <Navigation className="w-4 h-4 text-white" />
                  <span className="text-sm font-semibold text-white">Маршрут</span>
                </button>
                
                <button
                  onClick={() => window.open(mapLinks.twoGis, '_blank')}
                  className="flex items-center justify-center gap-2 h-10 bg-gradient-to-r from-orange-500 to-orange-600 rounded-full shadow-sm hover:shadow-md transition-all active:scale-95"
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-white">
                    <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
                  </svg>
                  <span className="text-sm font-semibold text-white">2GIS</span>
                </button>
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-white">Такси</span>
                </button>
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-white">Автобус</span>
                </button>
              </div>

              {/* Long Toggle Switch for 4 Actions - Desktop only */}
              <div className="hidden lg:block relative w-full h-12 bg-gray-300 rounded-full p-1 mb-3">
                <div 
                  className={`absolute top-1 bottom-1 bg-gradient-to-r from-orange-500 to-orange-600 rounded-full shadow-md transition-all duration-300 ease-in-out ${
                    selectedAction === 'route' ? 'left-1 w-[calc(25%-4px)]' :
                    selectedAction === '2gis' ? 'left-[calc(25%+1px)] w-[calc(25%-4px)]' :
                    selectedAction === 'taxi' ? 'left-[calc(50%+1px)] w-[calc(25%-4px)]' :
                ...
                <div className="relative flex h-full">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 text-sm font-semibold transition-colors duration-300 ${
                      selectedAction === 'route' ? 'text-white' : 'text-gray-600'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 text-sm font-semibold transition-colors duration-300 ${
                      selectedAction === '2gis' ? 'text-white' : 'text-gray-600'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setSelectedAction('taxi')}
                    className={`flex-1 flex items-center justify-center gap-1.5 text-sm font-semibold transition-colors duration-300 ${
                      selectedAction === 'taxi' ? 'text-white' : 'text-gray-600'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setSelectedAction('bus')}
                    className={`flex-1 flex items-center justify-center gap-1.5 text-sm font-semibold transition-colors duration-300 ${
                      selectedAction === 'bus' ? 'text-white' : 'text-gray-600'
                    }`}
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M4 16c0 .88.39 1.67 1 2.22V20c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h8v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1.78c.61-.55 1-1.34 1-2.22V6c0-3.5-3.58-4-8-4s-8 .5-8 4v10zm3.5 1c-.83 0-1.5-.67-1.5-1.5S6.67 14 7.5 14s1.5.67 1.5 1.5S8.33 17 7.5 17zm9 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm1.5-6H6V6h12v5z"/>
              ...
              {/* Transport Info */}
              {selectedAction === 'taxi' && (
                <div className="mt-3 p-3 bg-white rounded-xl text-xs text-gray-700 leading-relaxed border border-gray-200">
                  <p className="mb-2">
              ...
              <div className="p-4 border-t border-gray-200 bg-white">
                <div className="flex items-center gap-2">
                  <a
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 bg-gray-50 rounded-xl hover:bg-blue-50 transition-all group"
                  >
                    ...
                    <span className="text-xs font-semibold text-gray-700 group-hover:text-[#4285F4]">Google</span>
                  </a>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 bg-gray-50 rounded-xl hover:bg-red-50 transition-all group"
                  >
                    <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-red-600">
                      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
                    </svg>
                    <span className="text-xs font-semibold text-gray-700 group-hover:text-red-600">Yandex</span>
                  </a>
                </div>
              </div>
            </div>
